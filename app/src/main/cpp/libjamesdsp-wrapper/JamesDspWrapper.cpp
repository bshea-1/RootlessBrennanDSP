...
Java_me_timschneeberger_rootlessjamesdsp_interop_JamesDspWrapper_free(JNIEnv *env, jobject obj, jlong self)
{
    ...
    if (currentEnv == nullptr && wrapper->jvm != nullptr) {
        jint res = wrapper->jvm->GetEnv(reinterpret_cast<void**>(&currentEnv), JNI_VERSION_1_6);
        if (res == JNI_EDETACHED) {
            wrapper->jvm->AttachCurrentThread(&currentEnv, nullptr);
            didAttach = true;
        }
    }

    if (currentEnv != nullptr && wrapper->callbackInterface != nullptr) {
        currentEnv->DeleteGlobalRef(wrapper->callbackInterface);
    }
    if (didAttach && wrapper->jvm != nullptr) {
...
Java_me_timschneeberger_rootlessjamesdsp_interop_JamesDspWrapper_processInt16(JNIEnv *env, jobject obj, jlong self, jshortArray inputObj, jshortArray outputObj, jint offset, jint size)
{
    ...
    for (jsize i = 0; i < inputLength; i++) {
        ...
        else if (output[i] < -32768) output[i] = -32768;
    }
...
extern "C"
JNIEXPORT void JNICALL
Java_me_timschneeberger_rootlessjamesdsp_interop_JamesDspWrapper_processDirectBuffer(JNIEnv *env, jobject obj, jlong self, jobject inputObj, jobject outputObj, jint frameCount)
{
    DECLARE_DSP_V

    if (inputObj == nullptr || outputObj == nullptr || frameCount <= 0)
        return;

    auto* inPtr = static_cast<float*>(env->GetDirectBufferAddress(inputObj));
    auto* outPtr = static_cast<float*>(env->GetDirectBufferAddress(outputObj));

    if (inPtr == nullptr || outPtr == nullptr)
        return;

    dsp->processFloatMultiplexd(dsp, inPtr, outPtr, frameCount);

    const int totalSamples = frameCount * 2;
    for (int i = 0; i < totalSamples; i++) {
        if (outPtr[i] > 1.0f) outPtr[i] = 1.0f;
        else if (outPtr[i] < -1.0f) outPtr[i] = -1.0f;
    }
}
                                                                             ...
                                                                             jboolean enable, jint mode, jint customFcut, jint customFeed)
{
    ...
    if(mode == 99)
    {
        ...
        BS2BInit(&dsp->advXF.bs2b[0], (unsigned int)dsp->fs, ((unsigned int)customFcut | ((unsigned int)customFeed << 16)));
...
void receiveLiveprogStdOut(const char *buffer, void* userData)
{
    ...
    if(self == nullptr || self->jvm == nullptr)
    {
        LOGE("JamesDspWrapper::receiveLiveprogStdOut: Self reference or JVM is NULL");
    ...
    bool didAttach = false;
    jint res = self->jvm->GetEnv(reinterpret_cast<void**>(&callbackEnv), JNI_VERSION_1_6);
    if (res == JNI_EDETACHED) {
        self->jvm->AttachCurrentThread(&callbackEnv, nullptr);
        didAttach = true;
    }

    if (callbackEnv != nullptr && self->callbackInterface != nullptr && self->callbackOnLiveprogOutput != nullptr) {
        jstring str = callbackEnv->NewStringUTF(buffer);
        callbackEnv->CallVoidMethod(self->callbackInterface, self->callbackOnLiveprogOutput, str);
        callbackEnv->DeleteLocalRef(str);
    }

    if (didAttach && self->jvm != nullptr) {
