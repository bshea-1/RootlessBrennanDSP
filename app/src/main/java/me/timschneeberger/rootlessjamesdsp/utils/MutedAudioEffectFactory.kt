            ...
            muteEffect = when (type) {
                MuteEffects.DynamicsProcessing -> {
                    sdkAbove<AudioEffect?>(VERSION_CODES.P) {
                        ...
                        with(DynamicsProcessing(Int.MAX_VALUE, sid, config)) {
                            setInputGainAllChannelsTo(-96f)
                            try {
                                val limiter = getLimiterByChannelIndex(0)
                                limiter.isEnabled = true
                                limiter.postGain = -96f
                                limiter.threshold = -96f
            ...
            if (!enabled) {
                try {
                    if(SdkCheck.isPie && effect is DynamicsProcessing)
                        effect.setInputGainAllChannelsTo(-96f)
                    effect.enabled = true
