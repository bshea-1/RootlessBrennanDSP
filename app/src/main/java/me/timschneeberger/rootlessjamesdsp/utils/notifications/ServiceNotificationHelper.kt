...
object ServiceNotificationHelper: KoinComponent {
    ...
    fun pushServiceNotification(context: Context, sessions: Array<IEffectSession>, mediaSessionToken: MediaSession.Token? = null) {
        context.getSystemService<NotificationManager>()
            ?.notify(Notifications.ID_SERVICE_STATUS, createServiceNotification(context, sessions, mediaSessionToken))
    }
    ...
    fun createServiceNotification(context: Context, sessions: Array<IEffectSession>, mediaSessionToken: MediaSession.Token? = null): Notification {
    ...
    private fun createServiceNotification(
        ...
        if (mediaSessionToken != null) {
            val builder = Notification.Builder(context, Notifications.CHANNEL_SERVICE_STATUS)
                .setShowWhen(false)
            ...
            if (isRootless()) {
                val stopIntent = PendingIntent.getService(
                    context, 0,
                    Intent(context, RootlessAudioProcessorService::class.java).apply {
                        action = RootlessAudioProcessorService.ACTION_STOP
                    },
                    PendingIntent.FLAG_ONE_SHOT or PendingIntent.FLAG_IMMUTABLE
                )
                builder.addAction(Notification.Action.Builder(
                    android.graphics.drawable.Icon.createWithResource(context, R.drawable.ic_close_24dp),
