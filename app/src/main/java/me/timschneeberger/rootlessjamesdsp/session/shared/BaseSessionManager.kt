    ...
    BroadcastReceiver(), MediaSessionManager.OnActiveSessionsChangedListener, KoinComponent
{
    ...
    protected abstract fun handleSessionDump(sessionDump: ISessionInfoDump?, policyDump: ISessionPolicyInfoDump?)
    protected open fun onActivePlaybackDetected(sid: Int, uid: Int, packageName: String) {}
    ...
    init {
        ...
        audioPlaybackCallback = object : AudioManager.AudioPlaybackCallback() {
            override fun onPlaybackConfigChanged(configs: MutableList<AudioPlaybackConfiguration>?) {
                ...
                Timber.d("Playback config changed: ${configs?.size ?: 0} configs")
    ...
    private fun handlePlaybackConfigs(configs: List<AudioPlaybackConfiguration>) {
        configs.forEach { config ->
            try {
                val refined = Refine.unsafeCast<AudioPlaybackConfigurationHidden>(config)
                val sid = refined.sessionId
                val uid = refined.clientUid
                if (sid > 0 && uid > 0 && uid != Process.myUid()) {
                    val packageName = context.getPackageNameFromUid(uid) ?: uid.toString()
                    val isPlayerActive = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                        try { refined.isActive } catch (_: Throwable) { true }
                    } else true

                    if (isPlayerActive) {
                        Timber.d("Real-time active playback detected: sid=$sid; uid=$uid; pkg=$packageName")
                        onActivePlaybackDetected(sid, uid, packageName)
                    }
                }
            } catch (e: Exception) {
                Timber.w(e, "handlePlaybackConfigs: could not parse AudioPlaybackConfiguration")
            }
        }
    }
    ...
    private suspend fun pollSessionDump(blocking: Boolean = true)
    {
        ...
        if (!blocking) {
            // Keep debounce tight (300ms) so rapid track changes in TikTok/Instagram
            // are processed immediately without dropping session mute updates.
            if (now - lastPollTimestamp < 300L) {
                return
            }
        }
        ...
        pollingMutex.withLock {
            lastPollTimestamp = SystemClock.elapsedRealtime()
            val sessionDump = dumpManager.dumpSessions()
            val policyDump = dumpManager.dumpCaptureAllowlistLog()
            withContext(Dispatchers.Main) {
                handleSessionDump(sessionDump, policyDump)
            }
        }
    }
